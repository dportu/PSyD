...
void Task8(void* id){
	INT8U err;
	    ...
	    OSSemPend( uart0Sem, 0, &err );    /* Muestra un mensaje inicial en la UART0 (protegida por un semáforo) */
	        lcd_puts( 20,32,BLACK,"  Task" );
	        lcd_putchar(20+7*8,32,BLACK, *(char *)id );
	        lcd_puts( 20+10*8,32,BLACK," iniciada.\n" );
	    OSSemPost( uart0Sem );
	...
	while(1){
		...
		lcd_puts( 20,32,BLACK,"  (Task 8) Tecla pulsada: " );
		lcd_puthex(27*8,32,BLACK,scancode);
		OSSemPost( uart0Sem );
	}
}
void Task9(void*id ){
	...
	OSSemPend( uart0Sem, 0, &err );    /* Muestra un mensaje inicial en la UART0 (protegida por un semáforo) */
		lcd_puts(20,16,BLACK, "  Task" );
		lcd_putchar( 28,16,BLACK,*(char *)id );
		lcd_puts(30 ,16,BLACK," iniciada.\n" );
	lcd_draw_box( 10, 10, 310, 230, BLACK, 5 );
	OSSemPost( uart0Sem );
	...
	while(1)
	{
		...
		rtc_gettime( &rtc_time );
		lcd_draw_box( 10, 10, 310, 230, BLACK, 5 );
		lcd_puts(pos,16,BLACK,"  (Task 9) Segundos: " );
		//								  Hora:
		pos = pos + 20*8;
		lcd_putint(pos,16,BLACK,rtc_time.sec);
		if(rtc_time.sec>9)pos = pos +1*8;
		pos = pos +1*8;
		lcd_puts(pos,16,BLACK,"   ");
		OSSemPost( uart0Sem );
	}
}
/*******************************************************************/
